...
export function buildPixPayload(pix, amount) {
  ...
  const merchantName = normalizeAsciiText(pix.receiverName || pix.merchantName, 25) || 'Negocio Local';
  const merchantCity =
    normalizeAsciiText(pix.city || pix.merchantCity, 15, { uppercase: true }) || 'SAO PAULO';
  const pixAmount =
  ...
  const additionalInfoLimit = Math.max(0, 99 - merchantAccountInfoParts.join('').length - 4);
